<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-gray-800 leading-tight">Daftar Dekorasi</h2>
    </x-slot>

    <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <a href="{{ route('dekorins.create') }}"
            class="mb-6 inline-flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 transition duration-300 shadow">
            ...
            <table id="datatable" class="w-full text-sm text-gray-700">
                <thead class="bg-gray-100 text-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 border">Tema</th>
                ...
                <tbody>
                    @foreach ($dekorins as $dekorin)
                        <tr class="hover:bg-gray-50 transition duration-200">
                            ...
                            <td class="px-6 py-3">{{ $dekorin->tema }}</td>
                            <td class="px-6 py-3">{{ $dekorin->category->name ?? '-' }}</td>
                            <td class="px-6 py-3">Rp{{ number_format($dekorin->price ?? 0, 0, ',', '.') }}</td>
                            <td class="px-6 py-3 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('dekorins.edit', $dekorin->id) }}"
                                        class="px-4 py-1.5 bg-yellow-400 text-white text-sm font-semibold rounded hover:bg-yellow-500 transition">
                                        Edit
                                    </a>
                                    <form action="{{ route('dekorins.destroy', $dekorin->id) }}" method="POST"
